function ContentH(){
	var winH = $(window).height(),
			...
			val1 = winH - guide - btm;
	console.log(guide,btm);
	//$('body').css('padding-top', header+guide+'px');
	$('body').css('padding-top', guide+'px');
...
function activefriend(){ // 신규추가
	var user = $('.circleArea .user');
	$(user).not('.leader, .non').click(function(){
		if (!$(user).parent().hasClass('finish')){ // 완료화면에선 동작 X
			$(this).toggleClass('active').siblings().removeClass('active'); // 선택해제가 따로 없어 토글로 넣었습니다.
		}
	});
}

/* 2020.06.04 전민재 수정 Start */
function thumbSize(){ // 신규추가 : height값을 못찾는거같아 jQuery로 넣어놨습니다.
	var circle = $('.circleArea'),
			circle_w = $(circle).outerWidth(),
			user_w = circle_w * 0.25;
	/*var circle = $('.circleArea');
	//circle_w = $(circle).outerWidth(),
	circle_w = $(circle)[0].style.width;
	circle_w.replace('px','');
	user_w = circle_w * 0.25;*/
	
	$(circle).find('.thumb').css({
		'width' : user_w+'px',
		'height' : user_w+'px'
	});
}

/* 2020.06.04 전민재 수정 End */
...
function onBtn_conFirm5(target){
	$(target).attr('class', 'layerPop alert_yteen');
		...
		$.ajax({
			...
			success : function(data){
				...
				if(data.resultCd == "200") {
					...
					if(data.resultData.giftRcvYn == "Y"){
						$('[name=giftRcvYn]').val('Y');
						giftChk(target);
					}else{
						reLoad(emSeq);
					}
					//onPopup(themaSeq, emSeq, rmSeq, '', '#layerPop3', '');
				} else {
					onPopup(themaSeq, emSeq, rmSeq, '', '#errorPop1', data.resultMsg);
				}
			}
...
function onBtn_conFirm3(target){
		...
		if(themaGiftSeq == "" || themaGiftSeq == null){
			//alert("경품을 선택해주세요.");
			onPopup3(themaSeq, emSeq, rmSeq, '', '#errorPop6', "경품을 선택해주세요.");
			return false;
		}
		...
		if(addrUseYn == "Y"){
			if(post == "" || post == null){
				//alert("우편번호를 입력해주세요.");
				onPopup3(themaSeq, emSeq, rmSeq, '', '#errorPop6', "우편번호를 입력해주세요.");
				return false;
			}

			if(addr == "" || addr == null){
				//alert("주소를 입력해주세요.");
				onPopup3(themaSeq, emSeq, rmSeq, '', '#errorPop6', "주소를 입력해주세요.");
				return false;
			}

			if(addrDtl == "" || addrDtl == null){
				//alert("주소 상세를 입력해주세요.");
				onPopup3(themaSeq, emSeq, rmSeq, '', '#errorPop6', "주소 상세를 입력해주세요.");
				return false;
			}
		}
		...
		$.ajax({
			...
			url : "/yapp/yteenFriends/roomGift",
			data : {
				themaSeq : themaSeq,
				post : post,
				emSeq : emSeq,
			...
			success : function(data){
				...
				if(data.resultCd == "200") {
					...
					onPopup3(themaSeq, emSeq, rmSeq, '', '#errorPop6', data.resultMsg);
				}
			}
		});
...
function onBtn_conFirm4(target){
		...
		var cntrNo = $(":button").filter(".active").eq(0).find('[name=joinCntrNo]').val();
...
function onPopup2(joinSeq, themaSeq, emSeq, rmSeq, dataid, id){
		...
		$.ajax({
			...
			url : "/yapp/yteenFriends/themaGift",
			data : {
				themaSeq : themaSeq,
				joinSeq : joinSeq,
				emSeq : emSeq
				...
				if(data.resultCd == "200") {
					...
					for(var i=0; i<data.resultInfoList.length; i++) {
						...
						dataListHtml +='<label class="chk">'
							+'<input type="radio" name="choice" value="'+data.resultInfoList[i].giftName+'" data-seq="'+data.resultInfoList[i].giftSeq+'" data-type="'+data.resultInfoList[i].giftType+'" data-use="'+data.resultInfoList[i].addrUseYn+'" data-cnt="'+data.resultInfoList[i].remainCnt+'"/>'
					...
					if(totalCnt < 1){
						...
						$('#btn_confirm3').css('background', '#a9a9a9');
						$('#btn_confirm3').prop('disabled', true);
					}

					$(id).find('.event_title').addClass('group-yteen'); // title 변경 event_title group-YTEEN FRIENDS
...
function onPopup3(themaSeq, emSeq, rmSeq, thema, id, themaKor){

...
<head lang="en">
		...
		<!-- 경품복수 선태처리 관련 css 난수 변경 처리 -->
		<link type="text/css" rel="stylesheet" href="/yapp3/res/css/layout.css?20230612">
		...
		<style>
	    	...
	    	#not_info_multi_popup{display:none;}
...
$(document).ready(function(){
	...
	var eventGiftMaxChoice = "${eventDetail.giftMaxChoice}";//경품 복수선택 관련 최대 선택 가능 건수 추가 2023.06.12
	...
	//복수 경품 신청 관련 알림 팝업 닫기 처리
	$("#not_info_multi_ok").click(function(){
		scrollAble();
		$(this).parents('.popup').hide();
	});
	...
	//첫번째 라디오 버튼에 체크
	$("input:checkbox[name=giveaway]").eq(0).attr("checked",true).parents('li').addClass('on');
	//첫번째 라디오 체크된 텍스트값
	$('#data-resualt').text($("input:checkbox[name=giveaway]").eq(0).siblings('label').text());
	// 라디오 선택시 선택 라디오 해당 img opacity 조정 + 선택된 라디오로 텍스트 변경
	$('.radio-list-row input').click(function(){
		var giftSelCnt = $("input:checkbox[name=giveaway]:checked").length;
		if(giftSelCnt > eventGiftMaxChoice){
			//alert("최대 경품 응모 가능 건수는 "+eventGiftMaxChoice+"건 입니다.");
			scrollDisble();
			$("#not_info_multi_popup").find("p").text("최대 경품 응모 가능 건수는 "+eventGiftMaxChoice+"건 입니다.");
			$("#not_info_multi_popup").show();
			return false;
		}
		console.log($(this).parents('li').hasClass("on"));
        ...
        if(giftSelCnt >= 1){
        	$(".radio-list-resualt").html("경품으로 <span id=\"data-resualt\"></span>를 선택하셨습니다.");
        }
		// 복수 선택한 경품에 대한 정보 표시 
		if(giftSelCnt > 1){
			var selGiftLabel = "";
			$("input:checkbox[name=giveaway]:checked").siblings('label').each(function(index,item){
				var gubun = ", ";
				if(giftSelCnt == index + 1){
					gubun = "";
				}
				selGiftLabel += $(this).text() + gubun;
				console.log(index+","+ $(this).text());
				
			});
			$('#data-resualt').text(selGiftLabel);
			//var selectedMultiGift = $('#data-resualt').text() + ","+selectedGift;
			
		} else if(giftSelCnt == 1) {
			$('#data-resualt').text($("input:checkbox[name=giveaway]:checked").siblings('label').text());
		} else {
			//$('#data-resualt').text("경품이 선택되지 않았습니다.");
			$(".radio-list-resualt").html("경품을 선택하지 않았습니다.")
		}
	...
	//경품선택 확인 버튼 클릭시 
	var eventOnLineGiftJoinList = [];
	$("#giftChoice_complete").click(function(){
		eventOnLineGiftJoinList = [];
		var evtType="${eventDetail.evtType}";
		var giftChoiceType="${eventDetail.giftChoiceType}";	//경품 제공 방식 (선택,랜덤)
		var giftOfferType="${eventDetail.giftOfferType}";//O0003추첨, O0002선착순, O0001상품없음
		...
		var giftType="G0002";
		if(giftSelCnt == 0){
			//alert("경품을 선택하지 않았습니다.");
			$("#not_info_multi_popup").find("p").text("경품을 선택하지 않았습니다.");
			$("#not_info_multi_popup").show();
			return false;
		}
		...
		if(memStatus=="G0003" && assoAddrYn == "Y"){
			...
			if(giftType=="G0002" || giftType == "G0003"){
				$("#delivery_popup").find(".f-yspotlightotapp").text("정보를 입력해 주세요");
			}else{
				$("#delivery_popup").find(".f-yspotlightotapp").text("배송지 정보를 입력해 주세요");
			}
			//고객정보 입력
		...
		var isOffLineGift = false;
		var isOnLineGift = false;
		$("input:checkbox[name=giveaway]:checked").each(function(){
			const giftType = $(this).siblings('input:hidden[name=giftType]').val();
			if(giftType == "G0001"){
				isOffLineGift = true;
		...
		if(isOffLineGift){
			...
			if(isOnLineGift){
				...
				$("input:checkbox[name=giveaway]:checked").each(function(){
					var giftType=$(this).siblings('input:hidden[name=giftType]').val();
					if(giftType == "G0002"){
					var giftSeq=$(this).siblings('input:hidden[name=giftSeq]').val();
					var issueSeq=$(this).siblings('input:hidden[name=issueSeq]').val();
					...
					var eventGiftJoin={};
						...
						eventOnLineGiftJoinList.push(eventGiftJoin);
					}
				});
		...
		if(evtType=='G0002' || evtType=='G0008'/*210611 투표하기 추가*/){
			if(doubleSubmitCheck()) return;
			var gifOnReqJson = {"evtSeq":evtSeq,"giftMaxCnt":eventGiftMaxChoice,"eventGiftJoinList":eventOnLineGiftJoinList};
			console.log(JSON.stringify(gifOnReqJson));
			if(giftType=="G0002"){
			  	$.ajax({
					type : "post",
					headers : {
					...
					url : "giftRewardJoinMulti",
					data :  JSON.stringify(gifOnReqJson),
	...
	$("#delivery_complete").click(function(){
		...
		var offLineGiftCnt = $("input:checkbox[name=giveaway]:checked").length;
		console.log("offLineGiftCnt==>"+offLineGiftCnt);
		console.log("giftChoiceType==>"+giftChoiceType);
		var giftType="G0002";
		if(offLineGiftCnt == 0){
			//alert("경품을 선택하지 않았습니다.");
			$("#not_info_multi_popup").find("p").text("경품을 선택하지 않았습니다.");
		...
		// 사용자 정보 미입력시, validation 체크 - Start
		if(addrFormYn=="Y"){
					...
					var eventGiftJoin={"evtSeq":evtSeq
									  ,"giftSeq":giftSeq
							 	      ,"issueSeq":issueSeq
							  		  ...
							  		  ,"remarks":remarks
							  		  ,"giftType":giftType};
					eventGiftJoinList.push(eventGiftJoin);
		...
		if(memStatus=="G0003" && assoAddrYn=="Y"){
			var eventGiftJoinList = [];
			var giftType=$("input:checkbox[name=giveaway]:checked").siblings('input:hidden[name=giftType]').val();
			$("input:checkbox[name=giveaway]:checked").each(function(){
				var giftSeq=$(this).siblings('input:hidden[name=giftSeq]').val();
				var issueSeq=$(this).siblings('input:hidden[name=issueSeq]').val();
				var eventGiftJoin={"evtSeq":evtSeq
			...
			if(giftType=="G0001"){
				...
				if(evtType=='G0002' || evtType=='G0008' /*210614 투표하기 추가*/){
					console.log("OUT eventOnLineGiftJoinList ==> "+eventOnLineGiftJoinList);
					if(giftOfferType == "O0003" && (eventOnLineGiftJoinList != null && eventOnLineGiftJoinList.length > 0)){
						//eventAllGiftJoinList = eventGiftJoinList.concat(eventOnLineGiftJoinList);
						$.each(eventOnLineGiftJoinList,function(index,item){
							eventGiftJoinList.push(item);
						});
					} 
					if(doubleSubmitCheck()) return;
					var gifOffReqJson = {"evtSeq":evtSeq,"giftMaxCnt":eventGiftMaxChoice,"eventGiftJoinList":eventGiftJoinList};
					console.log(JSON.stringify(gifOffReqJson));
					$.ajax({
						type : "post",
						headers : {
							cntrNo : $('[name=cntrNo]').val(),
						...
						url : "giftOfflineJoinMulti",
						data :  JSON.stringify(gifOffReqJson),
						contentType:"application/json;charset=UTF-8",
						success : function(data){
							if(data.resultCd=="200"){
								console.log("data : "+JSON.stringify(data));
								...
								if(giftChoiceType=="C0001"){	//경품 제공 방식 : 선택
									console.log("IN eventOnLineGiftJoinList ==> "+eventGiftJoinList);
										scrollDisble();
										$("#giftChoice_complete_popup").show();
								}
			...
			//온라인상품 || 응모권상품
				...
				if(doubleSubmitCheck()) return;
				var gifOnReqJson = {"evtSeq":evtSeq,"giftMaxCnt":eventGiftMaxChoice,"eventGiftJoinList":eventGiftJoinList};
				console.log(JSON.stringify(gifOnReqJson));
				$.ajax({
					type : "post",
					headers : {
				...
				$.ajax({
					...
					success : function(data){
						if(data.resultCd=="200"){
							...
							if(giftChoiceType=="C0001"){	//경품 제공 방식 : 선택
								console.log("IN eventOnLineGiftJoinList ==> "+eventOnLineGiftJoinList);
									scrollDisble();
									$("#giftChoice_complete_popup").show();
							}
...
<!-- EVE-01-POP9_경품선택 -->
		...
		<div class="pop-bt">
			...
			<!--경품 선택  -->
				<c:if test="${eventDetail.giftChoiceType eq 'C0001'}">
					...
					<p class="f-yspotlightotapp">경품은 최대 ${eventDetail.giftMaxChoice}개까지 선택 가능합니다.</p>
							...
							<li>
								<label for="giveaway${status.count }"><img src="${giftList.imgUrl}" alt="" width="154px;" height="154px;"></label>
									<div class="checkbox-custom-wrap">
										<input type="checkbox" name="giveaway" id="giveaway${status.count }"
	...
	<!-- EVE-01-POP5_1_정보미입력 -->
	<div id="not_info_multi_popup" class="popup" style="z-index:4;">
		<!-- 배경 커버(블랙) -->
		<div class="cover"></div>
		<!-- //배경 커버(블랙) -->
		<!-- 팝업 내용 -->
		<div class="pop-bt">
			<div class="pop-con">
				<p>최대 경품 응모 가능 건수는 ${eventDetail.giftMaxChoice}건 입니다.</p>
			</div>
			<div class="pop-btn-wrap bg-darkgray">
				<button type="button" id="not_info_multi_ok" class="btn w100p" onfocus="blur()" >확인</button>
				<!-- button에 btn-high 클래스를 추가하면 아래 여백이 커집니다 -->
			</div>
		</div>
		<!-- //팝업 내용 -->
